...
find_vendored_act_bin() {
  local candidate=""

  candidate="$(cd "$repo_root/../../../../Utilities/src_and_submodules/utilities/management_of/resources/servers/local/act/act/dist/local" 2>/dev/null && pwd || true)"
  if [[ -n "$candidate" && -x "$candidate/act" ]]; then
    printf '%s\n' "$candidate/act"
    return 0
  fi
...
ensure_act() {
  ...
  vendored_act_bin="$(find_vendored_act_bin || true)"
  if [[ -n "$vendored_act_bin" ]]; then
